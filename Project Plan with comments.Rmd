...
```{block}
The description uses the term "causes" of coronavirus cases which seems a stretch. I suspect you are really looking for relationships or associations between cornonavirus occurence and/or outcomes and other factors.

Given the two data sets, there are options for both trend analysis and totals analysis since the CDC data is only totals unless you have some historical data. 
...
```{block}
I would be explicit about whether you plan to use the daily totals or just the totals data for the period.
```
...
Recommend on the About tab you include the option to get the latest data that if requested triggers a download and cleaning and storage of the latest data. You shoudl also show the date of the latest data on the about tab.

Not a lot of detail on the other tabs to support your design.
...
```{block}
There is no reason to use pie charts as there are better options for displaying categorical data.
